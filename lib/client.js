	...
	factory: (require) => {
		...
		function recompute(state, snapshot, opts = {}) {
			...
			for (const id of Object.keys(byId)) {
				...
				// 5) Live answerable wait (question / approval / plan-review). It
				//    exists only while unanswered; answering resolves it and the
				//    run resumes — blue then clears by itself.
				if (s.pendingInteraction !== undefined && s.pendingInteraction !== null) {
					blue = true;
				}
			}
			...
			ctx.effect(() => {
				...
				function applySnapshot() {
					...
					try {
						...
						const { green, blue } = recompute(state, snap, { pageHidden: effectiveHidden, clearIds });
						persist();
						...
						if (snap && snap.phase !== "pending") {
							...
							if (pageHidden) {
								...
								if (dwellStartAt === 0) {
									...
									const { green, blue } = recompute(state, snap, { pageHidden: false });
									persist();
									lastFlags = [green, blue];
									schedule(stateFor(green, blue));
								}
		...
		// Required services (service names, awaited before apply runs).
